...
st.set_page_config(page_title="PokéBattle 23.0 (Retreat Fix)", page_icon="⚔️", layout="wide")
...
if st.session_state.tela_ranking:
            ...
            for p in reversed(dados_brutos):
                with st.expander(f"{p['data']} - 🏆 {p['vencedor']} (vs {p['perdedor']})"):
                    ...
                    if 'log' in p and p['log']:
                        ...
                        st.caption("Log detalhado não disponível.")
    ...
    def render_player(key):
        ...
        if p['banco']:
            st.markdown("---")
            # --- FIX CRASH BANCO ---
            cols = st.columns(len(p['banco']) if p['banco'] else 1) # Dinâmico
