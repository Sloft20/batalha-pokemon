...
# Se não tiver o arquivo, ele usa os dados de backup abaixo para não quebrar
try:
...
st.set_page_config(page_title="PokéBattle 41.1 (Syntax Fix)", page_icon="⚔️", layout="wide")
...
class Pokemon:
    def __init__(self, nome, hp_max, tipo, fraqueza, resistencia, recuo, imagem_url="", habilidade=None):
        ...
        # Tenta pegar habilidade do DB se não for passada
        if habilidade:
            self.habilidade = habilidade
        else:
            self.habilidade = POKEDEX.get(nome, {}).get("hab")
    ...
    def resolver_checkup(self):
        ...
        if self.status == "Envenenado 🧪":
            ...
            if random.choice(["CARA", "COROA"]) == "CARA":
                self.status = "Saudável"
                logs.append(f"🪙 {self.nome} curou queimadura!")
            else:
                logs.append(f"🪙 {self.nome} continua queimado.")
        elif self.status == "Adormecido 💤":
            if random.choice(["CARA", "COROA"]) == "CARA":
                self.status = "Saudável"
                logs.append(f"🪙 {self.nome} acordou!")
            else:
                logs.append(f"🪙 {self.nome} dormindo.")
        return logs
...
def verificar_custo_ataque(pokemon):
    ...
    if sum(pool.values()) >= incolores_nec:
        return True
    return False
